...
void* validate_address(const void*);
...
syscall_handler (struct intr_frame *f UNUSED) 
{
  ...
  int * p = f->esp;
  validate_address(p);
  //validate_address(p+1);
  //printf("size:%d",sizeof(p_1));
  //validate_address(p_1+sizeof(p_1));
  ...
  switch (system_call)
  {
	...
	case SYS_CREATE:
	validate_address(p+5);
	validate_address(*(p+4));
	...
	case SYS_READ:
	validate_address(p+7);
	validate_address(*(p+6));
	if(*(p+5)==0)
	{
		int i;
		uint8_t* buffer = *(p+6);
...
void exit_proc(int status)
{
	...
	if(thread_current()->parent->waiting_ch_tid == thread_current()->tid)
		sema_up(&thread_current()->parent->child_lock);

	thread_exit();
}

void* validate_address(const void *vaddr)
{
