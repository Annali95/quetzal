...
start_process (void *file_name_)
{
  ...
  if (!success) {
    printf("%d %d\n",thread_current()->tid, thread_current()->exit_error);
...
process_wait (tid_t child_tid) 
{
  ...
  for (e = list_begin (&cur_t->child_proc); e != list_end (&cur_t->child_proc);e = list_next (e))
  {
  ...
  if(!ch || !e1)    ///returns -1 immediately, without waiting.
    return -1;

  cur_t->waiting_ch_tid = ch->tid;
    
  if(!ch->used)
    sema_down(&cur_t->child_lock);     //Waits for thread TID to die 
